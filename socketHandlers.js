    ...
    io.on('connection', (socket) => {
        ...
        socket.on('joinRoom', ({ playerName, roomCode, playerToken }) => {
            ...
            if (session?.roomCode === roomCodeUpper) {
                ...
                if (existingPlayer) {
                    ...
                    if (game.state === 'lobby') { socket.emit('joinSuccess', { roomCode: roomCodeUpper }); }
                        ...
                        const allThemeLocations = gameManager.getAvailableLocations(game.settings.themes);
                        const payload = { round: game.currentRound, totalRounds: game.settings.rounds, players: game.players, isSpectator: existingPlayer.isSpectator, allPlayerRoles: game.players.filter(p=>!p.isSpectator).map(p=>({id:p.id, role:parseRole(p.role).name})), allLocations: game.spyLocationList, allLocationsData: allThemeLocations, canBountyHunt: game.settings.bountyHuntEnabled };
            ...
            if (game.state !== 'lobby') {
                ...
                const allThemeLocations = gameManager.getAvailableLocations(game.settings.themes);
                const payload = { round: game.currentRound, totalRounds: game.settings.rounds, players: game.players, isSpectator: true, allPlayerRoles: game.players.filter(p=>!p.isSpectator).map(p=>({id:p.id, role:parseRole(p.role).name})), allLocations: game.spyLocationList, allLocationsData: allThemeLocations, location: game.currentLocation, role: "ผู้ชม" };
                socket.emit('gameStarted', payload);
                io.to(roomCodeUpper).emit('updatePlayerList', {players: game.players, settings: game.settings});
                return;
            }
        ...
        socket.on('toggleSpectatorMode', () => {
            ...
            if (game && player && !player.isHost && game.state === 'lobby') {
                // If player is currently a spectator (true or 'waiting'), they become a player (false).
                // Otherwise, they become a spectator (true).
        ...
        socket.on('resetGame', () => { const { game, player } = getCurrentState(); if (game && player?.isHost) { game.state = 'lobby'; game.currentRound = 0; game.locationDeck = []; game.players.forEach(p => { p.score = 0; p.isSpectator = false; }); gameManager.clearTimers(game); io.to(socket.roomCode).emit('returnToLobby'); io.to(socket.roomCode).emit('updatePlayerList', {players: game.players, settings: game.settings}); } });
