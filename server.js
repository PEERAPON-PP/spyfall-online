...
function getAvailableLocations(theme){ if (theme==='default') return locationsData.filter(l=>l.category==='default'); if (theme==='fairytale') return locationsData.filter(l=>l.category==='fairytale'); return locationsData; }
function pickSpyOptions(allNames, currentName, count=25){ const pool=allNames.filter(n=>n!==currentName); shuffleArray(pool); const picks=pool.slice(0, Math.max(0,count-1)); picks.push(currentName); shuffleArray(picks); return picks; }
...
io.on('connection', (socket)=>{
  ...
  socket.on('rejoinGame', (playerToken)=>{
    ...
    // เติมรายการสถานที่ให้ spy ที่รีจอย
  ...
  // --- Request spy list ซ้ำ (กดเปิด modal ภายหลัง)
  ...
  // Host skip: playing → เปิดโหวต, voting/revoting → คำนวณทันที
  ...
  socket.on('submitVote', (votedPlayerId)=>{
    ...
    if (Object.keys(game.votes).length === active){
...
function calculateVoteResults(roomCode){
  ...
  if (top.length===1 && spyInTop){
    ...
    game.state='revoting'; game.votes={}; game.revoteCandidates = game.players.filter(p=>top.includes(p.id)).filter(p=>!p.disconnected);
